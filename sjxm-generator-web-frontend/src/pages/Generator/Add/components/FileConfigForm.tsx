...
export default (props: Props) => {
   ...
   * @param remove
   */
  const singleFieldFormView = (field: FormListFieldData, remove?: (index: number | number[]) => void) => (
    <Space>
  ...
  return (
    <>
      <Alert message="如果不需要使用在线制作功能，可以不填写" type="warning" closable/>
      <div style={{marginBottom: 16}}></div>
      <Form.List name={["fileConfig", "files"]}>
        {(fields, {add, remove}) => (
          <div style={{display: 'flex', rowGap: 16, flexDirection: 'column'}}>
            {fields.map((field) => {
              //注意要获取到groupKey，防止修改时识别分组错误
